      ...
      <div className="p-4 space-y-4">
        {agent && (agent.roles || []).length > 0 && (
          <div className="space-y-1">
            <label className="text-[10px] uppercase font-bold tracking-wider text-slate-400 dark:text-neutral-400">角色/风格</label>
            <CustomSelect
        ...
        <button
          ...
          className={`nodrag w-full mt-2 py-2 text-[10px] font-bold rounded-lg border transition-all active:scale-95 flex items-center justify-center gap-2 ${
            executing || loadingAgent || !agent || !prompt.trim() || (data as any)._canEdit === false
            ...
            <div className="grid grid-cols-4 gap-2">
              {referenceImages.map((u, i) => (
                <div key={i} className="relative group">
                  <img src={u} alt="ref" className="w-full h-12 object-cover rounded-md border border-slate-200 dark:border-neutral-800 group-hover:border-neutral-400 dark:group-hover:border-neutral-400 transition-colors" />
                </div>
              ))}
            </div>
          </div>
        ...
        {referenceDocuments.length > 0 && (
          <div className="space-y-1">
            <div className="text-[10px] uppercase font-bold tracking-wider text-slate-400 dark:text-neutral-400">参考文档 ({referenceDocuments.length})</div>
            <div className="space-y-1">
              {referenceDocuments.map((d, i) => (
                <div key={i} className="flex items-center gap-2 text-xs text-slate-600 dark:text-slate-300">
                  <span className="material-symbols-outlined text-slate-400 dark:text-neutral-400" style={{ fontSize: '14px', fontVariationSettings: '"FILL" 0, "wght" 200, "GRAD" 0, "opsz" 20' }}>description</span>
