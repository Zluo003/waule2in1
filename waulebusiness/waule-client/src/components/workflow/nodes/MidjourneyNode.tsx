...
const MidjourneyNode = ({ data, selected, id }: NodeProps<MidjourneyNodeData>) => {
        ...
        try {
          ...
          if (task.status === 'SUCCESS') {
            ...
            if (data.imageUrl) {
              ...
              generatedImageUrl = processedResult.displayUrl;
            }
            ...
            toast.error(`任务失败: ${task.failReason || '未知错误'}`);
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      if (foundParam) {
        toast.error(`请不要在提示词中添加参数 "${foundParam}"，请使用下方的配置选项`, { duration: 4000 });
        setStatus('idle');
        updateNodeData({ status: 'idle', progress: '', taskId: '' });
        return;
      }
      ...
      try {
        ...
        if (translationResponse.success) {
          ...
          if (translationResponse.needsTranslation) {
            ...
            toast.success(`已自动翻译（检测到${translationResponse.detectedLanguage === 'zh' ? '中文' : '非英文'}）`, { duration: 3000 });
          } else {
        ...
        // 翻译失败不阻止生成，继续使用原文
        toast.warning('翻译服务不可用，将使用原始提示词', { duration: 3000 });
      }

      toast.info('正在提交 Midjourney 任务...');
      ...
      if (omniReferenceImages.length > 0) {
        toast.info(`正在上传 ${omniReferenceImages.length} 张参考图到 Discord...`);
        ...
        for (let i = 0; i < omniReferenceImages.length; i++) {
          ...
          try {
            ...
            toast.error(`参考图 ${i + 1} 上传失败: ${error.message}`);
            setStatus('idle');
            return;
          }
        }
        ...
        toast.success('Omni-Reference 参考图上传完成！');
      }

      // V7 Style-Reference: 上传风格参考图到 Discord 并添加 --sref 参数
      if (styleReferenceImages.length > 0) {
        toast.info(`正在上传 ${styleReferenceImages.length} 张风格参考图到 Discord...`);
        ...
        for (let i = 0; i < styleReferenceImages.length; i++) {
          ...
          try {
            ...
            toast.error(`风格参考图 ${i + 1} 上传失败: ${error.message}`);
            setStatus('idle');
            return;
          }
        }
        ...
        toast.success('Style-Reference 风格参考图上传完成！');
      }
      ...
      if (respIsFree) {
        ...
        toast.success(`任务已提交（已扣除 ${respCredits} 积分）`);
        refetchEstimate();
      } else {
        toast.success(`任务已提交: ${newTaskId}`);
      }
      ...
      if (error.response?.status === 403) {
        const errMsg = error.response?.data?.error || '您没有权限使用 Midjourney';
        toast.error(errMsg);
        return;
      }
      ...
      if (bannedWord) {
        toast.error(`敏感词检测: "${bannedWord}" - 请修改提示词`, { duration: 5000 });
      } else {
        toast.error(`任务提交失败: ${errorMsg}`);
      }
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const pollTaskStatus = useCallback(async (taskId: string) => {
    let attempts = 0;
    const maxAttempts = 300; // 最多10分钟 (300 * 2秒)
    ...
    const poll = async () => {
      try {
        ...
        setProgress('');
        updateNodeData({ status: 'error', progress: '', taskId: '' });
        toast.error(`轮询失败: ${error.message}`);
      }
    };
          ...
          <div className="space-y-1">
            ...
            <div className="nodrag flex gap-2" onPointerDown={(e) => e.stopPropagation()} onMouseDown={(e) => e.stopPropagation()}>
              ...
              <button
                ...
                onClick={() => {
                  if (!fastModeEnabled) {
                    toast.warning('Fast模式已经用完，目前仅支持Relax模式，敬请见谅。');
                    return;
                  }
                  setMode('fast');
                }}
