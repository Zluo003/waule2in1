    ...
    try {
      ...
      try {
        ...
        if (parent) {
          ...
          // 本地存储模式：将音频上传到本地服务器
          let audioUrl = previewBlobUrlRef.current || '';
          if (audioUrl && isLocalStorageEnabled()) {
            const localServerUrl = getLocalServerUrl();
            if (localServerUrl) {
              try {
                const response = await fetch(audioUrl);
                const audioBlob = await response.blob();
                const fileName = `audio-design-${Date.now()}.mp3`;
                ...
                if (uploadResponse.ok) {
                  ...
                  if (result.localUrl) {
                    ...
                    console.log('[AudioDesign] 音频已上传到本地服务器:', audioUrl);
                  }
                }
              } catch (uploadError) {
                console.warn('[AudioDesign] 上传到本地失败，使用blob URL:', uploadError);
              }
            }
          }
          
          setNodes((nds) => [...nds, { id: newId, type: 'audioPreview', position: { x: posX, y: parent.position.y }, data: { audioUrl, sourceNodeId: id, createdBy: (parent.data as any)?.createdBy } } as any]);
