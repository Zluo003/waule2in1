...
const SuperCanvasNode = ({ data, id, selected }: NodeProps<SuperCanvasNodeData>) => {
    ...
    return (
        <div ref={containerRef} className={`relative flex flex-col w-[800px] h-auto bg-white/80 dark:bg-[#18181b]/100 dark:backdrop-blur-none backdrop-blur-sm border rounded-2xl shadow-xl transition-all ring-1 ${selected ? 'border-neutral-400 shadow-neutral-400/50' : 'border-slate-200 dark:border-neutral-800 ring-black/5 dark:ring-neutral-700 ring-black/5'}`}>
            <style>
                {`
            ...
            <div className="flex-1 flex flex-col gap-4 p-4">
                {/* Canvas Area */}
                <div ref={canvasContainerRef} className="relative w-full aspect-square bg-slate-100 dark:bg-black rounded-xl overflow-hidden shadow-inner border border-slate-200 dark:border-neutral-800">
                ...
                <div className="h-12 flex items-center gap-2">
                    {/* Tools Section */}
                    <div className="flex-1 flex items-center gap-1 bg-slate-100 dark:bg-black p-1 rounded-lg overflow-x-auto scrollbar-hide border border-slate-200 dark:border-neutral-800">
                        {[
                            ...
                            <button
                                ...
                                className={`p-2 rounded-lg transition-all ${activeTool === tool.id
                                    ? 'bg-neutral-500/20 text-neutral-500 dark:text-neutral-400 shadow-sm'
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <input
                                ...
                                className="super-color-input w-4 h-4 rounded cursor-pointer border border-slate-200 dark:border-neutral-800"
                                title="Brush Color"
                            />
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <input
                                ...
                                className="nodrag w-10 h-2 rounded-lg appearance-none cursor-pointer"
                                style={{
                                    background: `linear-gradient(to right, #404040 0%, #737373 ${((brushSize - (activeTool === 'brush' ? 80 : 10)) / ((activeTool === 'brush' ? 400 : 30) - (activeTool === 'brush' ? 80 : 10))) * 50}%, #06b6d4 ${((brushSize - (activeTool === 'brush' ? 80 : 10)) / ((activeTool === 'brush' ? 400 : 30) - (activeTool === 'brush' ? 80 : 10))) * 100}%, var(--range-bg-color, #e2e8f0) ${((brushSize - (activeTool === 'brush' ? 80 : 10)) / ((activeTool === 'brush' ? 400 : 30) - (activeTool === 'brush' ? 80 : 10))) * 100}%, var(--range-bg-color, #e2e8f0) 100%)`
                                }}
                                title="Brush Size"
                            />
                        ...
                        <button 
                            ...
                            try {
                                ...
                                if (isLocalStorageEnabled()) {
                                    ...
                                    if (localServerUrl) {
                                        ...
                                        if (response.ok) {
                                            ...
                                            if (result.localUrl) {
                                                finalImageUrl = result.localUrl;
                                                console.log('[SuperCanvas] 图片已上传到本地服务器:', finalImageUrl);
                                            }
                                        }
                                    }
                                } else {
                                ...
                                console.warn('[SuperCanvas] 上传失败，使用 base64:', uploadError);
                            ...
                            className="nodrag px-3 py-2 text-[10px] font-bold rounded-lg border transition-all active:scale-95 flex items-center justify-center whitespace-nowrap bg-neutral-800 dark:bg-white text-white dark:text-black shadow-md hover:shadow-lg border-transparent dark:border-neutral-700 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:shadow-md"
