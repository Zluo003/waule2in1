  ...
  return (
    <div className={`relative bg-white/80 dark:bg-[#18181b]/100 dark:backdrop-blur-none backdrop-blur-sm border rounded-2xl shadow-xl transition-all ring-1 ${selected ? 'border-neutral-400 shadow-neutral-400/50' : 'border-white/60 dark:border-neutral-700 ring-black/5 dark:ring-neutral-700 ring-black/5'}`} style={{ width: 320 }}>
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="space-y-1">
          <label className="text-[10px] uppercase font-bold tracking-wider text-slate-400 dark:text-neutral-400">语言</label>
          <CustomSelect
            value={language}
        ...
        <div className="flex items-center gap-2">
          <div className="flex-1 space-y-1">
            <label className="text-[10px] uppercase font-bold tracking-wider text-slate-400 dark:text-neutral-400">音色</label>
            <CustomSelect
              value={voiceId}
          ...
          <button onClick={() => handleSynthesize(true)} disabled={isBusy} className={`mt-auto w-9 h-9 rounded-full flex items-center justify-center text-white hover:shadow-lg ${isBusy ? 'bg-gray-600 dark:bg-gray-700' : ''}`} style={!isBusy ? { background: document.documentElement.classList.contains('dark') ? 'linear-gradient(to right, #4b1b77, #6f153f)' : 'linear-gradient(to right, #737373, #404040)' } : undefined}>
            <span className="material-symbols-outlined text-base">play_arrow</span>
          </button>
        </div>
        <div className="space-y-1">
          <label className="text-[10px] uppercase font-bold tracking-wider text-slate-400 dark:text-neutral-400">合成文本</label>
          <textarea
            ref={textRef}
            value={text}
        ...
        <button onClick={() => handleSynthesize(false)} disabled={isBusy || (data as any)._canEdit === false} className={`nodrag w-full mt-2 py-2 text-[10px] font-bold rounded-lg border transition-all active:scale-95 flex items-center justify-center gap-2 text-white shadow-md hover:shadow-lg ${isBusy || (data as any)._canEdit === false ? 'bg-gray-600 dark:bg-gray-700' : 'border-transparent dark:border-neutral-700'}`} style={!(isBusy || (data as any)._canEdit === false) ? { background: document.documentElement.classList.contains('dark') ? 'linear-gradient(to right, #4b1b77, #6f153f)' : 'linear-gradient(to right, #737373, #404040)' } : undefined}>
          {isBusy ? '合成中...' : '生成语音'}
        </button>
      </div>
