...
const AssetLibraryDetailPage = () => {
  ...
  const { config: _storageConfig } = useTenantStorageStore();
  ...
  useEffect(() => {
    const loadRolePreviewImages = async () => {
      try {
        ...
        // 优先使用 metadata 中保存的 URL
        const assets: Asset[] = [];
        if (m.images?.faceUrl) {
          assets.push({ id: m.images?.faceAssetId || 'face', name: '面部', url: m.images.faceUrl, type: 'IMAGE' } as Asset);
        }
        if (m.images?.frontUrl) {
          assets.push({ id: m.images?.frontAssetId || 'front', name: '主图', url: m.images.frontUrl, type: 'IMAGE' } as Asset);
        }
        if (m.images?.sideUrl) {
          assets.push({ id: m.images?.sideAssetId || 'side', name: '参考1', url: m.images.sideUrl, type: 'IMAGE' } as Asset);
        }
        if (m.images?.backUrl) {
          assets.push({ id: m.images?.backAssetId || 'back', name: '参考2', url: m.images.backUrl, type: 'IMAGE' } as Asset);
        }
        
        // 如果没有 URL，尝试从数据库获取（兼容旧数据）
        if (assets.length === 0) {
          const ids = [m.images?.faceAssetId, m.images?.frontAssetId, m.images?.sideAssetId, m.images?.backAssetId]
            .filter(Boolean) as string[];
          if (ids.length > 0 && !ids[0]?.startsWith('local_')) {
            const results = await Promise.all(ids.map((aid) => apiClient.assets.getById(aid).catch(() => null)));
            const fetchedAssets = results.map((r: any) => r?.data).filter(Boolean) as Asset[];
            setRolePreviewImages(fetchedAssets);
            return;
          }
        }
  ...
  useEffect(() => {
    const loadEditRoleImages = async () => {
      try {
        ...
        // 优先使用 metadata 中保存的 URL，避免查询不存在的本地资产 ID
        const assets: Asset[] = [];
        if (m?.images?.frontUrl) {
          assets.push({ id: m?.images?.frontAssetId || 'front', name: '主图', url: m.images.frontUrl, type: 'IMAGE' } as Asset);
        }
        ...
        if (assets.length === 0) {
          ...
          if (ids.length > 0 && !ids[0]?.startsWith('local_')) {
            const results = await Promise.all(ids.map((aid) => apiClient.assets.getById(aid).catch(() => null)));
            const fetchedAssets = results.map((r: any) => r?.data).filter(Boolean) as Asset[];
            setEditRoleImages(fetchedAssets);
            return;
          }
        }
  ...
  const loadAssets = async () => {
    try {
      ...
      if (library?.category === 'ROLE') {
        const res = await apiClient.assetLibraries.roles.list(id!) as any;
        setRoles(res.data || []);
        setAssets([]);
        // 保存权限信息（角色列表也返回权限）
        if (res.permissions) {
        ...
        const response = await apiClient.assetLibraries.getAssets(id!) as any;
  ...
  return (
    <div className="p-8">
      ...
      {showCreateRole && library?.category === 'ROLE' && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[10000] no-drag p-4">
          <div className="bg-white/80 dark:bg-black/60 backdrop-blur-xl border-2 border-slate-200 dark:border-white/10 rounded-2xl px-6 py-5 w-full max-w-xl md:max-w-2xl shadow-xl max-h-[90vh] overflow-y-auto custom-scrollbar">
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => setShowCreateRole(false)} className="p-2 hover:bg-slate-200 dark:hover:bg-white/10 rounded">
              ...
              <button
                onClick={async () => {
                  try {
                    ...
                    const frontRes = await uploadOne(roleMainFile, `${roleForm.name}-主图`);
                    const sideRes = await uploadOne(roleOpt1File, `${roleForm.name}-参考1`);
                    const backRes = await uploadOne(roleOpt2File, `${roleForm.name}-参考2`);
                    if (frontRes) { uploads.frontAssetId = frontRes.id; urls.frontUrl = frontRes.url; }
                    if (sideRes) { uploads.sideAssetId = sideRes.id; urls.sideUrl = sideRes.url; }
                    if (backRes) { uploads.backAssetId = backRes.id; urls.backUrl = backRes.url; }
                    // 移除配音与文档上传
                    const payload: any = { name: roleForm.name.trim(), ...uploads, ...urls };
