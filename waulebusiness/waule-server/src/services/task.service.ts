...
class TaskService {
  ...
  async createTask(params: CreateTaskParams) {
    ...
    if (!permissionResult.isFree) {
      ...
      logger.info(`[TaskService] 扣费参数:`, billingParams);
      try {
  ...
  private async processTask(taskId: string, model: any) {
    try {
      ...
      if (task.type === 'IMAGE') {
        ...
        // 调试日志：查看返回结果类型
        logger.info(`[TaskService] processImageTask 返回结果:`, {
          isArray: Array.isArray(imageResult),
          length: Array.isArray(imageResult) ? imageResult.length : 1,
  ...
  private async processImageTask(task: any, model: any): Promise<string | string[]> {
    ...
    logger.info(`[TaskService] 生成图片, 提供商: ${provider}, 模型: ${model.modelId}, 分辨率: ${imageSize || '默认'}`);
    ...
    if (provider === 'google') {
      ...
      const maxImages = task.metadata?.maxImages; // SeeDream 4.5 组图数量
  ...
  private async processVideoTask(task: any, model: any): Promise<string> {
    ...
    if (provider === 'minimaxi' || provider === 'hailuo' || provider === '海螺') {
      ...
      const minimaxMeta: any = task.metadata || {};
      const minimaxDuration = typeof minimaxMeta.duration === 'number' ? minimaxMeta.duration : 5;
      const minimaxResolution = typeof minimaxMeta.resolution === 'string' ? minimaxMeta.resolution : '1080P';
      ...
      const resolution = typeof meta.resolution === 'string' ? meta.resolution : '720p';
      ...
      if ((task.generationType || '') === '对口型') {
        ...
        const wanxMeta: any = task.metadata || {};
        const wanxDuration = typeof wanxMeta.duration === 'number' ? wanxMeta.duration : 5;
        const wanxResolution = typeof wanxMeta.resolution === 'string' ? wanxMeta.resolution : '1080P';
        ...
        const videoUrl = await wanxService.generateVideoFromFirstFrame({
          ...
          duration: wanxDuration,
          resolution: wanxResolution,
        });
        return videoUrl;
      }
