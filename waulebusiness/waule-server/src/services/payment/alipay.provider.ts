...
export class AlipayProvider implements PaymentProvider {
   ...
   * 创建支付二维码
   */
  async createQRCode(params: CreateOrderParams): Promise<QRCodeResult> {
    try {
      const bizContent = {
      ...
      logger.info(`[Alipay] 创建预下单请求: ${params.orderNo}`);
      ...
      if (response.alipay_trade_precreate_response) {
        ...
        if (result.code === '10000') {
          ...
          logger.error(`[Alipay] 预下单失败: ${result.code} - ${result.msg} - ${result.sub_msg}`);
          return {
            success: false,
            outTradeNo: params.orderNo,
      ...
      logger.error(`[Alipay] 创建二维码异常:`, error);
      return {
        success: false,
        outTradeNo: params.orderNo,
  ...
  async queryStatus(orderNo: string): Promise<PaymentStatus> {
    try {
      ...
      logger.error(`[Alipay] 查询订单状态异常:`, error);
      return {
        success: false,
        status: 'PENDING',
        errorMessage: error.message,
  ...
  async handleCallback(data: any): Promise<CallbackResult> {
    try {
      ...
      logger.error(`[Alipay] 处理回调异常:`, error);
      return {
        success: false,
        orderNo: data.out_trade_no || '',
        status: 'FAILED',
  ...
  async refund(orderNo: string, amount: number, reason?: string): Promise<RefundResult> {
    try {
      ...
      logger.error(`[Alipay] 退款异常:`, error);
      return {
        success: false,
        errorMessage: error.message,
      };
  ...
  verifyCallback(data: any): boolean {
    try {
      ...
      logger.error(`[Alipay] 验证签名异常:`, error);
      return false;
    }
  }
   ...
   * 构建请求参数
   */
  private buildRequestParams(method: string, bizContent: any): Record<string, string> {
  ...
  private async sendRequest(params: Record<string, string>): Promise<any> {
    ...
    try {
      ...
      logger.error(`[Alipay] 响应解析失败: ${text}`);
