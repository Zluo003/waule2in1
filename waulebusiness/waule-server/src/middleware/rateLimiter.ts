...
export const generalLimiter = rateLimit({
  ...
  handler: (req: Request, res: Response) => {
    logger.warn(`[RateLimit] IP ${getClientIp(req)} 触发通用限流`);
    res.status(429).json({
      success: false,
      message: '请求过于频繁，请稍后再试',
    });
...
export const authLimiter = rateLimit({
  ...
  handler: (req: Request, res: Response) => {
    logger.warn(`[RateLimit] IP ${getClientIp(req)} 触发认证限流`);
    res.status(429).json({
      success: false,
      message: '登录尝试过于频繁，请1分钟后再试',
    });
...
export const smsLimiter = rateLimit({
  ...
  handler: (req: Request, res: Response) => {
    logger.warn(`[RateLimit] IP ${getClientIp(req)} 触发短信限流`);
    res.status(429).json({
      success: false,
      message: '验证码发送过于频繁，请1分钟后再试',
    });
 ...
 * 每小时短信限制（防止短信轰炸）
 */
export const smsHourlyLimiter = rateLimit({
  ...
  handler: (req: Request, res: Response) => {
    logger.warn(`[RateLimit] IP ${getClientIp(req)} 触发小时短信限流`);
    res.status(429).json({
      success: false,
      message: '今日验证码发送次数已达上限，请1小时后再试',
    });
...
export const aiLimiter = rateLimit({
  ...
  keyGenerator: (req: Request) => {
    // 使用用户ID作为限流键（需要认证）
    return `ai-${(req as any).user?.id || getClientIp(req)}`;
  },
  handler: (req: Request, res: Response) => {
    logger.warn(`[RateLimit] 用户 ${(req as any).user?.id || getClientIp(req)} 触发AI限流`);
    res.status(429).json({
      success: false,
      message: 'AI 请求过于频繁，请稍后再试',
    });
