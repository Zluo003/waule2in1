...
// 🚀 Socket.io JWT 认证中间件（支持平台版和租户版用户）
io.use(async (socket, next) => {
  try {
    ...
    const decoded = jwt.verify(token, secret) as { 
      ...
      identifier?: string; 
      role?: string;
      tenantId?: string;      // 租户版特有
      type?: string;          // 租户版: 'tenant_user'
    };
    
    // 判断是租户版还是平台版用户（租户版 token 有 type: 'tenant_user'）
    const isTenantUser = decoded.type === 'tenant_user' && !!decoded.tenantId;
    const actualUserId = decoded.userId; // 两种版本都用 userId 字段
    
    // 🔧 优先从 Redis 缓存获取用户信息（避免每次连接都查询数据库）
    const cacheKey = `socket:user:${isTenantUser ? 'tenant:' : ''}${actualUserId}`;
    let user: { id: string; isActive: boolean; role: string; nickname?: string | null; avatar?: string | null; tenantId?: string } | null = null;
    ...
    if (!user) {
      if (isTenantUser) {
        ...
        const tenantUser = await prisma.tenantUser.findUnique({
          ...
          select: { id: true, isActive: true, isAdmin: true, nickname: true, avatar: true, tenantId: true },
        });
        if (tenantUser) {
          user = {
            id: tenantUser.id,
            isActive: tenantUser.isActive,
            role: tenantUser.isAdmin ? 'ADMIN' : 'USER', // 转换为统一的 role 格式
    ...
    if (!user || !user.isActive) {
      logger.warn(`[Socket] 连接被拒绝: 用户不存在或已禁用 ${actualUserId}`);
      return next(new Error('用户不存在或已被禁用'));
    }
    
    // 将用户信息附加到 socket（租户版额外包含 tenantId）
    (socket as any).user = { 
      id: user.id, 
      role: user.role, 
      nickname: user.nickname,
      avatar: user.avatar,
      tenantId: isTenantUser ? decoded.tenantId : undefined,
      isTenantUser,
    };
    // 🔇 减少日志输出，仅在 debug 模式下记录
    if (process.env.LOG_LEVEL === 'debug') {
      logger.info(`[Socket] 用户 ${user.id} 认证成功 (${isTenantUser ? '租户版' : '平台版'})`);
    }
    next();
  } catch (error: any) {
...
io.on('connection', (socket) => {
  ...
  socket.on('join-workflow', async (workflowId: string) => {
    const isTenantUser = user.isTenantUser;
    
    // 🔧 优先从 Redis 缓存检查权限
    const permissionCacheKey = `workflow:permission:${isTenantUser ? 'tenant:' : ''}${workflowId}:${user.id}`;
    let hasAccess = false;
    ...
    if (!hasAccess) {
      if (isTenantUser) {
        // 租户版用户：检查租户工作流权限
        const tenantWorkflow = await prisma.tenantWorkflow.findFirst({
          where: {
            id: workflowId,
            OR: [
              { tenantUserId: user.id },
              { collaborators: { some: { tenantUserId: user.id } } },
              // 检查项目级别的协作者（用于剧集工作流）
              { project: { collaborators: { some: { tenantUserId: user.id } } } },
            ],
          },
          select: { id: true },
        });
        hasAccess = !!tenantWorkflow || user.role === 'ADMIN';
      } else {
        // 平台版用户：检查平台工作流权限
    ...
    if (process.env.LOG_LEVEL === 'debug') {
      logger.info(`[Socket] ${socket.id} 加入房间: workflow:${workflowId} (${isTenantUser ? '租户版' : '平台版'})`);
    }
    ...
    if (userInfo) {
      ...
      // 发送加入成功确认
      socket.emit('joined-workflow', { workflowId, userId: userInfo.id });
      
      // 广播更新后的在线用户列表
      await broadcastOnlineUsers(workflowId);
      
      logger.info(`[Socket] ${socket.id} 用户 ${userInfo.nickname || userInfo.id} 加入工作流 ${workflowId}`);
    }
  });
  ...
  socket.on('user:join', async (data: { workflowId: string }) => {
    let userInfo: { id: string; nickname: string | null; avatar: string | null } | null = null;
...
const corsOptions: cors.CorsOptions = {
  origin: (origin, callback) => {
    // 允许全域 CORS（仅在 ALLOW_ALL_CORS=true 时生效）
    if (allowAllCors) {
  ...
  allowedHeaders: ['Authorization', 'Content-Type', 'Accept', 'Origin', 'X-Requested-With', 'X-Tenant-ID'],
...
app.use(`${API_PREFIX}/admin/tenants`, tenantRoutes); // 租户管理
app.use(`${API_PREFIX}/tenant-auth`, tenantAuthRoutes); // 租户用户认证
app.use(`${API_PREFIX}/client`, clientRoutes); // 客户端激活
app.use(`${API_PREFIX}/tenant`, tenantApiRoutes); // 租户用户 API（项目、工作流、AI等）
app.use(`${API_PREFIX}`, nodePromptRoutes); // 节点提示词管理（包含 /admin/node-prompts 和 /node-prompts）
...
// Admin SPA 路由回退（所有 /admin/* 请求返回 index.html）
app.get('/admin/*', (req: Request, res: Response) => {
  res.sendFile(path.join(adminDistPath, 'index.html'));
});

// Client 前端静态文件服务（企业版客户端）
const clientDistPath = path.join(__dirname, '../dist/client');
app.use(express.static(clientDistPath, {
  etag: true,
  maxAge: isDevelopment ? 0 : '1d',
}));
// Client SPA 路由回退（所有非 API、非 admin 请求返回 index.html）
...
const startScheduledTasks = () => {
  ...
  memoryCheckInterval = setInterval(() => {
    ...
    // 🚨 内存警告阈值：单实例 1GB
    if (heapUsedMB > 1000) {
      logger.error(`[内存告警] 堆内存过高: ${heapUsedMB}MB / ${heapTotalMB}MB，RSS: ${rssMB}MB`);
      // 触发垃圾回收（如果可用）
      if (global.gc) {
        logger.warn('[内存告警] 触发手动 GC');
        global.gc();
      }
    } else if (heapUsedMB > 700) {
      logger.warn(`[内存监控] 堆内存较高: ${heapUsedMB}MB / ${heapTotalMB}MB，RSS: ${rssMB}MB`);
    }
  }, 5 * 60 * 1000); // 5分钟
...
const startServer = async () => {
  try {
    ...
    (httpServer as any).setTimeout(600000);      // 10分钟总超时
    (httpServer as any).headersTimeout = 60000;  // 1分钟请求头超时
    (httpServer as any).requestTimeout = 600000; // 10分钟请求体超时
  } catch (error) {
...
const gracefulShutdown = async () => {
  ...
  try {
    ...
    // Discord 连接已移至 waule-api 管理，无需在此关闭
