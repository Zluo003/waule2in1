...
import { upload, uploadAsset, getAssets, getAsset, updateAsset, deleteAsset, listRecycleBin, restoreAsset, permanentDeleteAsset, recordRecycleItem, getPresignedUrl, confirmDirectUpload } from '../controllers/asset.controller';
...
router.get('/proxy-download', async (req: Request, res: Response) => {
  try {
    ...
    // 国内CDN不走代理
    const isChinaCdn = /oscdn2\.dyysy\.com|soraapi\.aimuse\.club|\.aliyuncs\.com/i.test(rawUrl);
    logger.info('代理下载资源:', { url: rawUrl.substring(0, 100), isChinaCdn });
    ...
    const tryReadLocal = async (raw: string) => {
      try {
        const u = new URL(raw);
        const host = req.get('host') || '';
        if (u.hostname === 'localhost' || u.hostname === '127.0.0.1' || u.hostname === host) {
          ...
          if (p.startsWith('/uploads/')) {
            ...
            if (fs.existsSync(fsPath)) {
              ...
              if (ext === '.mp3' || ext === '.mpeg') contentType = 'audio/mpeg';
    ...
    try {
      ...
      if (u.pathname.startsWith('/uploads/')) {
        ...
        const fsPath = pathMod.join(process.cwd(), rel);
        if (!fs.existsSync(fsPath)) {
          const ossUrl = await ensureAliyunOssUrl(fsPath);
          if (ossUrl && typeof ossUrl === 'string') {
            const resp2 = await axios.get(ossUrl, {
              responseType: 'arraybuffer',
              timeout: 60000,
              maxRedirects: 5,
              validateStatus: (status) => status >= 200 && status < 300,
    ...
    if (contentRangeHdr) res.setHeader('Content-Range', contentRangeHdr);
...
router.get('/proxy-stream', async (req: Request, res: Response) => {
  try {
    ...
    if (!contentType || contentType === 'application/octet-stream') {
      try {
        const head = await axios.head(finalUrl, { timeout: 15000, maxRedirects: 3, validateStatus: (s)=> s>=200 && s<400 });
    ...
    if (contentRange) res.status(206); else res.status(200);
...
router.get('/download/:id', async (req: Request, res: Response) => {
  try {
    ...
    // 确定下载文件名
    let downloadName = (filename as string) || asset.name;
    ...
    logger.info(`下载资产: ${asset.name} (${downloadName}) from ${assetUrl}`);
    ...
    logger.info(`资产下载成功，大小: ${buffer.length} bytes`);
...
router.post('/upload', upload.single('file'), handleMulterError, uploadAsset);
...
router.post('/transfer-url', async (req: Request, res: Response) => {
  try {
    ...
    logger.info('[转存] 服务器转存:', url.substring(0, 100));
    const ossUrl = await downloadAndUploadToOss(url, 'transfer');
    
    res.json({ success: true, data: { url: ossUrl } });
  } catch (error: any) {
    logger.error('[转存] 失败:', error.message);
...
router.delete('/:id/permanent', permanentDeleteAsset);
router.post('/recycle/record', recordRecycleItem);
