/**
 * Veo 3.1 系列视频生成模型配置
 * 通过 ai-gateway 中转调用 Google Veo 3.1 系列模型
 */
...
interface VeoModelsConfigProps {
  ...
  wauleServers: WauleApiServer[]
  onRefresh: () => void
}

export const VeoModelsConfig = ({ selectedModel, existingModels, wauleServers, onRefresh }: VeoModelsConfigProps) => {
  ...
  useEffect(() => {
    if (existing) {
      ...
      setServerId(existing.wauleApiServerId || '')
  ...
  const save = async () => {
    try {
      ...
      const payload = {
        ...
        wauleApiServerId: serverId || undefined,
        isActive,
      ...
      <div className="space-y-6">
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" value={name} onChange={e => setName(e.target.value)} className="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-border-dark bg-white dark:bg-slate-800 text-slate-900 dark:text-white" />
          </div>
          <div>
            <label className="block text-sm text-slate-600 dark:text-gray-400 mb-1">单次价格</label>
            <input type="text" value={price} onChange={e => setPrice(e.target.value)} placeholder="如: 0.5" className="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-border-dark bg-white dark:bg-slate-800 text-slate-900 dark:text-white" />
          </div>
        </div>

        <div>
          <label className="block text-sm text-slate-600 dark:text-gray-400 mb-1">Waule API 服务器</label>
          <select value={serverId} onChange={e => setServerId(e.target.value)} className="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-border-dark bg-white dark:bg-slate-800 text-slate-900 dark:text-white">
            <option value="">请选择服务器</option>
            {wauleServers.map(s => <option key={s.id} value={s.id}>{s.name} ({s.url}){s.isDefault ? ' [默认]' : ''}</option>)}
          </select>
          {wauleServers.length === 0 && <p className="text-xs text-red-500 mt-1">⚠️ 请先在上方添加 Waule API 服务器</p>}
        </div>
