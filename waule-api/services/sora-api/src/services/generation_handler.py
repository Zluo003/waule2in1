...
from .oss_utils import download_and_upload_to_oss
...
class GenerationHandler:
                            ...
                            if item.get("task_id") == task_id:
                                ...
                                if watermark_free_enabled:
                                    ...
                                    try:
                                        ...
                                        # Upload watermark-free video to OSS
                                        try:
                                            if stream:
                                                yield self._format_stream_chunk(
                                                    reasoning_content="Uploading video to OSS...\n"
                                                )
                                            local_url = await download_and_upload_to_oss(watermark_free_url, ".mp4", "sora", "video")
                                            if local_url and local_url != watermark_free_url:
                                                if stream:
                                                    yield self._format_stream_chunk(
                                                        reasoning_content="Video uploaded to OSS successfully. Preparing final response...\n"
                                                    )
                                            else:
                                                # OSS upload failed, use original URL
                                                local_url = watermark_free_url
                                                if stream:
                                                    yield self._format_stream_chunk(
                                                        reasoning_content="OSS upload failed, using original URL...\n"
                                                    )
                                        except Exception as oss_error:
                                            # Fallback to watermark-free URL if OSS upload fails
                                            local_url = watermark_free_url
                                            if stream:
                                                yield self._format_stream_chunk(
                                                    reasoning_content=f"Warning: Failed to upload to OSS - {str(oss_error)}\nUsing original watermark-free URL instead...\n"
                                                )

                                        # Delete the published post after upload
                                        try:
                                        ...
                                        # Upload to OSS
                                        try:
                                            local_url = await download_and_upload_to_oss(url, ".mp4", "sora", "video")
                                            if not local_url or local_url == url:
                                                local_url = url
                                        except Exception as oss_error:
                                            local_url = url
                                else:
                                    ...
                                    if url:
                                        ...
                                        if stream:
                                            yield self._format_stream_chunk(
                                                reasoning_content="**Video Generation Completed**\n\nVideo generation successful. Now uploading to OSS...\n"
                                            )

                                        try:
                                            local_url = await download_and_upload_to_oss(url, ".mp4", "sora", "video")
                                            if local_url and local_url != url:
                                                if stream:
                                                    yield self._format_stream_chunk(
                                                        reasoning_content="Video uploaded to OSS successfully. Preparing final response...\n"
                                                    )
                                            ...
                                            # Fallback to original URL if OSS upload fails
                                            local_url = url
                                            if stream:
                                                yield self._format_stream_chunk(
    ...
    async def _handle_character_creation_only(self, video_data, model_config: Dict) -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            # Step 7: Upload avatar to OSS and return success message with image
            avatar_url = profile_asset_url
            if profile_asset_url:
                try:
                    # 优先上传到 OSS
                    oss_url = await download_and_upload_to_oss(profile_asset_url, ".png", "sora-character")
                    if oss_url and oss_url != profile_asset_url:
                        avatar_url = oss_url
                        debug_logger.log_info(f"Avatar uploaded to OSS: {avatar_url}")
                    elif config.cache_enabled:
                        # 如果 OSS 上传失败，尝试本地缓存
                        cached_filename = await self.file_cache.download_and_cache(profile_asset_url, "image")
                        avatar_url = f"{self._get_base_url()}/tmp/{cached_filename}"
                        debug_logger.log_info(f"Avatar cached locally: {avatar_url}")
                except Exception as e:
                    debug_logger.log_info(f"Avatar upload/cache failed, using original URL: {e}")
