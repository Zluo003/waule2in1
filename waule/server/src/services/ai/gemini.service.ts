 ...
 */
function isProxyApiUrl(apiUrl?: string): boolean {
 ...
 */
async function generateImageViaProxyApi(options: GeminiImageGenerateOptions): Promise<string> {
  const {
    prompt,
  ...
  try {
    ...
    if (typeof messageContent === 'string') {
      ...
      const imagePart = messageContent.find((p: any) => p.type === 'image_url');
