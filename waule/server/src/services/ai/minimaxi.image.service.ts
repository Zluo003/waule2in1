...
async function downloadImage(url: string): Promise<string> {
  return downloadAndUploadToOss(url, 'minimaxi-image')
}

// 通过文件 ID 下载并上传到 OSS
async function downloadByFileId(baseUrl: string, apiKey: string, fileId: string): Promise<string> {
  const url = `${baseUrl}/files/retrieve_content?file_id=${encodeURIComponent(fileId)}`
  try {
    return await downloadAndUploadToOss(url, 'minimaxi-image', { Authorization: `Bearer ${apiKey}` })
  } catch {
    return url
  }
}
...
export async function generateImage(options: GenerateImageOptions): Promise<string> {
  ...
  const payload: any = { model: modelId, prompt, aspect_ratio: aspectRatio, n, response_format: 'url' }
  ...
  const resp = await axios.post(`${BASE}/image_generation`, payload, { headers: { Authorization: `Bearer ${KEY}`, 'Content-Type': 'application/json' }, timeout: 180000 })
  const data = resp.data || {}
  const genUrlsArr = (data?.data?.image_urls || data?.image_urls || data?.data?.images || data?.images)
  if (Array.isArray(genUrlsArr) && genUrlsArr.length > 0) return await downloadImage(String(genUrlsArr[0]))
  const genUrlStr = (data?.data?.image_url || data?.image_url || data?.data?.image_file || data?.image_file)
  if (typeof genUrlStr === 'string' && genUrlStr) return await downloadImage(String(genUrlStr))
  const images = data.data || data.images || []
  const fileId = data.file_id || data.data?.file_id
  if (fileId) return await downloadByFileId(BASE, KEY, String(fileId))
  if (Array.isArray(images) && images.length > 0) {
    const item = images[0]
    const url = item.url || item.image_url || item.data?.url
    const b64 = item.b64_json || item.base64
    if (url) return await downloadImage(String(url))
    if (b64) {
      // Base64 直接上传到 OSS
