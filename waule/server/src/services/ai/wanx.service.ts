...
async function uploadBufferToFallbackHost(buffer: Buffer, filename?: string): Promise<string> {
  ...
  const finalFilename = filename || `${crypto.randomUUID()}.jpg`;
...
interface WanxVideoGenerateOptions {
  ...
  apiKey?: string;
  apiUrl?: string;
  replaceImageUrl?: string; // 视频换人：人物图片URL
  replaceVideoUrl?: string; // 视频换人：参考视频URL
  mode?: 'wan-std' | 'wan-pro'; // 视频换人：服务模式
}
...
export async function generateVideoFromFirstFrame(options: WanxVideoGenerateOptions): Promise<string> {
  ...
  if (firstFrameImage) {
    ...
    if (firstFrameImage.startsWith('http://') || firstFrameImage.startsWith('https://')) {
      ...
      // 本地路径 - 上传到OSS
      const { uploadPath } = await import('../../utils/oss');
      let filePath: string;
  ...
  try {
    ...
    if (error.response?.data) {
      ...
      if ((errorMessage.includes('url error') || errorData.code === 'InvalidParameter') 
        ...
        try {
          // 下载图片并转换为Base64
        const imageResponse = await axios.get(processedFirstFrame, {
...
async function pollVideoTask(
  ...
  endpoint: string,
  apiKey: string,
  maxAttempts: number = 0 // 0 表示无限次，每次10秒
): Promise<string> {
...
export async function generateVideoRetalk(options: VideoRetalkOptions): Promise<string> {
  ...
  if (options.refImageUrl) requestBody.input.ref_image_url = options.refImageUrl;
  if (typeof options.videoExtension === 'boolean') requestBody.parameters.video_extension = options.videoExtension;
  const isFullEndpoint = /\/services\/aigc\//.test(rawApi);
  ...
  const createResponse = await axios.post(createTaskUrl, requestBody, { headers: requestHeaders, timeout: 30000 });
  const taskId = createResponse.data?.output?.task_id || createResponse.data?.task_id;
  if (!taskId) {
    throw new Error('未返回任务ID');
  }
...
export async function generateVideoStylize(options: VideoStylizeOptions): Promise<string> {
  ...
  if (typeof options.style === 'number') requestBody.parameters.style = options.style;
  if (typeof options.videoFps === 'number') requestBody.parameters.video_fps = options.videoFps;
  if (typeof options.minLen === 'number') requestBody.parameters.min_len = options.minLen;
  ...
  const createResponse = await axios.post(createTaskUrl, requestBody, { headers: requestHeaders, timeout: 60000 });
  const taskId = createResponse.data?.output?.task_id || createResponse.data?.task_id;
  if (!taskId) {
    throw new Error('未返回任务ID');
  }
