  ...
  if (wauleApiClient) {
    try {
      ...
      if (!imgMatch || !imgMatch[1]) {
        logger.error('[Sora] 无法从 waule-api 响应中提取图片URL:', content);
        throw new Error('Sora API响应中没有图片URL');
      }
      
      const imageUrl = imgMatch[1];
      logger.info(`[Sora] ✅ 图片生成成功 (waule-api)`, { imageUrl: imageUrl.substring(0, 80) });
      return imageUrl;
    } catch (error: any) {
      logger.warn(`[Sora] waule-api 调用失败，回退到直连: ${error.message}`);
    }
  }
  ...
  if (wauleApiClient) {
    try {
      ...
      if (!videoMatch || !videoMatch[1]) {
        logger.error('[Sora] 无法从 waule-api 响应中提取视频URL:', content);
        throw new Error('Sora API响应中没有视频URL');
      }
      
      const videoUrl = videoMatch[1];
      logger.info(`[Sora] ✅ 视频生成成功 (waule-api)`, { videoUrl: videoUrl.substring(0, 80) });
      return videoUrl;
    } catch (error: any) {
      logger.warn(`[Sora] waule-api 调用失败，回退到直连: ${error.message}`);
    }
 ...
 * 解析角色创建响应
 */
async function parseCharacterResponse(content: string): Promise<SoraCharacterResult> {
  // 优先解析 @username 格式的系统角色名（如 "角色名@abc123"）
  const atNameMatch = content.match(/角色名@(\w+)/);
  let characterName = '';
  if (atNameMatch) {
    characterName = `@${atNameMatch[1]}`;
    logger.info(`[Sora] 解析出的系统角色名: "${characterName}"`);
  } else {
    // 回退：解析中文字符作为角色名称
  ...
  logger.info(`[Sora] ✅ 角色创建成功！`, { characterName, avatarUrl: avatarUrl || '(无头像)' });
