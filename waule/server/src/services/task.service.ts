...
- Keep the same style, lighting and aspect ratio`;
  ...
  private async processVideoTask(task: any, model: any): Promise<string> {
    ...
    if (provider === 'minimaxi' || provider === 'hailuo' || provider === '海螺') {
      ...
      logger.info(`[TaskService] 🎬 开始处理 Veo 视频生成任务`);
      ...
      let allImages: string[] = [...(referenceImages || [])];
      ...
      if (frontendSubjects && frontendSubjects.length > 0) {
        ...
        logger.info(`[TaskService] Veo 从 subjects 获取图片: ${frontendSubjects.length}个角色`);
      }
      ...
      logger.info(`[TaskService] Veo 参数:`, {
        model: model.modelId,
        aspectRatio,
        enableUpsample,
        enhancePrompt,
      ...
      logger.info(`[TaskService] ✅ Veo 视频生成完成:`, videoUrl);
      return videoUrl;
    } else {
