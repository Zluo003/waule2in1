...
router.post(
  ...
  [
    ...
    body('modelId').notEmpty().withMessage('模型ID不能为空'),
    body('type').isIn(['TEXT_GENERATION', 'IMAGE_GENERATION', 'VIDEO_GENERATION', 'VIDEO_EDITING', 'AUDIO_SYNTHESIS']).withMessage('无效的模型类型'),
    body('config').custom((cfg) => {
      if (!cfg || typeof cfg !== 'object') return true;
      if (cfg.targetModel) throw new Error('请勿在 config 中提交 targetModel，与模型ID重复');
      return true;
    }),
  ],
