  ...
  try {
    ...
    try {
      ...
      if (cached === '1') {
        ...
        // 缓存标记为无效
        throw new AppError('登录已在其他设备失效，请重新登录', 401);
      }
    } catch (e) {
    ...
    if (!sessionValid) {
      ...
      if (!session) {
        // 缓存无效状态 30 秒
        try { await redis.set(sessionCacheKey, '0', 'EX', 30); } catch {}
        throw new AppError('登录已在其他设备失效，请重新登录', 401);
      }
      
      // 缓存有效状态 5 分钟（减少数据库查询）
      try { await redis.set(sessionCacheKey, '1', 'EX', 300); } catch {}
    }
    ...
    if (!user) {
      ...
      // 缓存用户信息 5 分钟（减少数据库查询）
      if (user) {
        try { await redis.set(cacheKey, JSON.stringify(user), 'EX', 300); } catch {}
      }
    }
...
export const optionalAuth = async (
  ...
  try {
    ...
    if (token) {
      ...
      try {
        const cached = await redis.get(sessionCacheKey);
        if (cached === '1') sessionValid = true;
        else if (cached === '0') return next(); // session 无效，跳过认证
      } catch {}
