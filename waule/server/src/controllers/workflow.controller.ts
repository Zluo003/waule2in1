...
export const getAllWorkflows = asyncHandler(async (req: Request, res: Response) => {
  ...
  // 构建所有查询 Promise
  const ownWorkflowsPromise = prisma.workflow.findMany({
    where,
    select: {
  ...
  // 处理共享的工作流
  let sharedWithMeta: any[] = [];
  let publicWithMeta: any[] = [];
  
  if (shouldIncludeShared) {
    sharedWithMeta = sharedWorkflows.map((w: any) => ({
      ...w,
      isOwner: false,
      isShared: true,
    ...
    // 过滤掉已经通过直接共享获取的
    const sharedIds = new Set(sharedWithMeta.map((w: any) => w.id));
    publicWithMeta = publicWorkflows
      .filter((w: any) => !sharedIds.has(w.id))
      .map((w: any) => ({
        ...w,
        isOwner: false,
        isShared: true,
...
export const getWorkflowById = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!workflow) {
    ...
    if (workflow) {
      try {
        await redis.set(cacheKey, JSON.stringify(workflow), 'EX', 30);
      } catch {
        // Redis 写入失败，忽略
      }
    }
  ...
  if (!workflow) {
    throw new AppError('工作流不存在', 404);
  }
  
  const isOwner = workflow.userId === userId;
  let canEdit = isOwner; // 所有者始终可编辑
  let sharePermission: string | undefined;
  ...
  if (!isOwner) {
    ...
    if (workflow.isPublic) {
      ...
      let share: { permission: string } | null = null;
      
      try {
        const cachedPerm = await redis.get(permCacheKey);
        if (cachedPerm) {
      ...
      if (!share) {
        share = await prisma.workflowShare.findUnique({
          where: {
        ...
        if (share) {
          try {
            await redis.set(permCacheKey, JSON.stringify(share), 'EX', 300);
          } catch {
            // Redis 写入失败，忽略
          }
        }
      ...
      if (!share) {
        throw new AppError('无权访问此工作流', 403);
      }
...
export const updateWorkflowById = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!canEdit) {
    throw new AppError('无权编辑此工作流', 403);
  }
...
export const getOrCreateShotWorkflow = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!episode) throw new AppError('剧集不存在', 404);
...
export const deleteWorkflow = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (workflow.userId !== req.user!.id) {
    throw new AppError('无权删除此工作流', 403);
  }
...
export const getWorkflowCollaborators = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!workflow) {
    throw new AppError('工作流不存在或无权访问', 404);
  }
 ...
 */
export const shareWorkflow = asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
  ...
  if (!workflow) {
    throw new AppError('工作流不存在或无权分享', 404);
  }
  ...
  if (!['READ', 'EDIT'].includes(permission)) {
    throw new AppError('无效的权限值', 400);
  }

  // 不能分享给自己
  if (targetUserId === userId) {
    throw new AppError('不能分享给自己', 400);
  }
  ...
  if (existingShare) {
    throw new AppError('已经分享给该用户', 400);
  }

  // 创建分享
  await prisma.workflowShare.create({
    data: {
      workflowId: id,
      ownerUserId: userId,
      targetUserId,
 ...
 * 更新协作者权限
 */
export const updateWorkflowSharePermission = asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
  const { targetUserId, permission } = req.body;
  const userId = req.user!.id;

  if (!targetUserId || !permission) {
    throw new AppError('请指定协作者和权限', 400);
  }
  ...
  if (!workflow) {
    throw new AppError('工作流不存在或无权操作', 404);
  }
  ...
  const share = await prisma.workflowShare.update({
    ...
    data: { permission: permission as 'READ' | 'EDIT' },
  });

  if (!share) {
    throw new AppError('协作者不存在', 404);
  }
 ...
 * 取消工作流分享
 */
export const unshareWorkflow = asyncHandler(async (req: Request, res: Response) => {
  const { id } = req.params;
 ...
 * 搜索用户（用于添加协作者）
 */
export const searchUsersForWorkflow = asyncHandler(async (req: Request, res: Response) => {
  const userId = req.user!.id;
  const { q } = req.query;
