  ...
  try {
    ...
    if (wauleApiClient) {
      ...
      if (providerLower === 'sora' || modelLower.includes('sora')) {
        ...
        if (!first) throw new Error('WauleAPI 未返回图片数据');
        imageUrl = first;
      }
    }
    ...
    if (!imageUrl) {
      throw new AppError('图片生成失败: 未获取到图片URL', 500);
    }
...
export const generateText = asyncHandler(async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!text) {
      throw new AppError('文本生成失败: 未获取到文本内容', 500);
    }
  ...
  try {
    ...
    if (wauleApiClient) {
      ...
      if (providerLower === 'sora' || modelLower.includes('sora')) {
          ...
          if (!videoMatch || !videoMatch[1]) throw new Error('WauleAPI Sora 响应中没有视频URL');
        ...
        const first = r?.data?.[0]?.url;
        if (!first) throw new Error('WauleAPI 未返回视频数据');
        videoUrl = first;
      }
    }
    ...
    if (!videoUrl) {
      throw new AppError('视频生成失败: 未获取到视频URL', 500);
    }
...
export const upscaleVideo = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!apiKey) {
    throw new AppError('缺少 Vidu API Key', 400);
  }
...
export const createCommercial = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (!apiKey && !apiUrl) {
    throw new AppError('缺少 Vidu API Key 或自定义服务器地址', 400);
  }
 ...
 * 获取支持图片编辑的模型列表
 */
export const getImageEditingModels = asyncHandler(async (req: Request, res: Response) => {
  ...
  // 过滤出支持图片编辑的模型
  const editingModels = models.filter((m: any) => {
 ...
 */
export const imageEdit = asyncHandler(async (req: Request, res: Response) => {
  const { prompt, mainImage, referenceImages, points, sourceImageDimensions } = req.body;
  const userId = req.user!.id;

  if (!mainImage) {
    throw new AppError('主图是必需的', 400);
  }

  if (!prompt) {
    throw new AppError('编辑指令是必需的', 400);
  }
  ...
  if (!permissionResult.allowed) {
    throw new AppError(permissionResult.reason || '您没有权限使用图片编辑功能', 403);
  }
  ...
  if (!permissionResult.isFree) {
    try {
      ...
      throw new AppError('扣费失败: ' + error.message, 400);
    }
  } else {
 ...
 */
export const identifyImagePoints = asyncHandler(async (req: Request, res: Response) => {
  const { image, points } = req.body;

  if (!image) {
    throw new AppError('图片是必需的', 400);
  }

  if (!points || points.length === 0) {
    throw new AppError('标记点是必需的', 400);
  }
 ...
 * 获取中文位置描述
 */
function getChinesePositionDescription(x: number, y: number): string {
  let horizontal = '';
  let vertical = '';

  if (x < 0.33) horizontal = '左侧';
  else if (x < 0.66) horizontal = '中间';
  else horizontal = '右侧';

  if (y < 0.33) vertical = '上方';
  else if (y < 0.66) vertical = '中部';
  else vertical = '下方';

  if (horizontal === '中间' && vertical === '中部') return '正中央';
