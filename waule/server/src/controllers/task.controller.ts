 ...
 * 创建图片编辑任务
 */
export const createImageEditTask = async (req: Request, res: Response) => {
  try {
    ...
    logger.info(`[TaskController] 创建图片编辑任务: ${prompt.substring(0, 50)}...`, {
    ...
    logger.error('[TaskController] 创建图片编辑任务失败:', error.message, error.stack);
    res.status(500).json({ error: errorMsg || '服务器内部错误' });
  }
};
