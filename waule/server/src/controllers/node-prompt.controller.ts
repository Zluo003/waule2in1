 ...
 */
export const initHDUpscaleTemplate = async (_req: Request, res: Response) => {
  try {
    // 检查是否已存在
    const existing = await nodePromptService.getByNodeType('hdUpscale');
    if (existing) {
      return res.json({ success: true, data: existing, message: '模板已存在' });
    }

    const defaults = nodePromptService.getHDUpscaleDefaults();
 ...
 */
export const initImageFusionTemplate = async (_req: Request, res: Response) => {
  try {
    // 检查是否已存在
    const existing = await nodePromptService.getByNodeType('imageFusion');
    if (existing) {
      return res.json({ success: true, data: existing, message: '模板已存在' });
    }

    const defaults = nodePromptService.getImageFusionDefaults();
    const template = await nodePromptService.create(defaults);
    res.status(201).json({ success: true, data: template, message: '初始化成功' });
  } catch (error: any) {
    console.error('初始化模板失败:', error);
 ...
 */
export const initSmartStoryboardTemplate = async (_req: Request, res: Response) => {
  try {
    // 检查是否已存在
    const existing = await nodePromptService.getByNodeType('smartStoryboard');
    if (existing) {
      return res.json({ success: true, data: existing, message: '模板已存在' });
    }

    const defaults = nodePromptService.getSmartStoryboardDefaults();
    const template = await nodePromptService.create(defaults);
    res.status(201).json({ success: true, data: template, message: '初始化成功' });
  } catch (error: any) {
    console.error('初始化模板失败:', error);
