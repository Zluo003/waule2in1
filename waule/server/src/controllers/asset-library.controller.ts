...
export const getLibraryAssets = async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      if (cached) {
        logger.info(`[Cache] Hit for library ${id}, returning cached data`);
        return res.json(JSON.parse(cached));
      }
    } catch (cacheError: any) {
      logger.warn(`[Cache] Failed to get cache for library ${id}: ${cacheError.message}`);
    }
    ...
    logger.info(`[Assets] Library ${id}: found ${filteredAssets.length} assets (raw: ${assets.length}, total: ${filteredTotal}, offset: ${offset}, limit: ${limit})`);
    ...
    try { 
      ...
      logger.info(`[Cache] Set cache for library ${id}`);
    } catch (cacheError: any) {
      logger.warn(`[Cache] Failed to set cache for library ${id}: ${cacheError.message}`);
    }
...
export const addAssetFromUrl = async (req: Request, res: Response) => {
  try {
    ...
    logger.info(`Asset added to library: ${asset.name} (${asset.id}) - libraryId: ${id}`);
    ...
    try { 
      ...
      const wildcardKeys = await redis.keys(`lib:assets:${id}:*`);
      const allKeys = [...new Set([...cacheKeysToDelete, ...wildcardKeys])];
      
      if (allKeys.length > 0) {
        const deletedCount = await redis.del(...allKeys);
        logger.info(`[Cache] Cleared ${deletedCount} cache keys for library ${id}`);
      }
    } catch (cacheError: any) {
      logger.warn(`[Cache] Failed to clear cache for library ${id}: ${cacheError.message}`);
    }
