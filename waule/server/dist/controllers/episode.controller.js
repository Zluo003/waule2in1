...
exports.updateEpisodePermission = exports.getEpisodeCollaborators = exports.deleteEpisode = exports.updateEpisode = exports.createEpisode = exports.getEpisode = exports.getEpisodes = void 0;
...
async function checkProjectAccess(projectId, userId) {
    ...
    const [project, share] = await Promise.all([
        index_1.prisma.project.findFirst({
            where: { id: projectId, userId },
            select: { id: true },
        }),
        index_1.prisma.projectShare.findFirst({
    ...
    // 缓存 2 分钟
    try {
        await index_1.redis.set(cacheKey, JSON.stringify(result), 'EX', 120);
    }
    catch { }
    return result;
}
...
async function checkEpisodeEditPermission(projectId, episodeId, userId) {
    // 检查是否是项目所有者
    ...
    // 检查是否有剧集编辑权限
    const permission = await index_1.prisma.episodePermission.findFirst({
...
const createEpisode = async (req, res) => {
    try {
        ...
        // 检查集数是否已存在
        const existingEpisode = await index_1.prisma.episode.findFirst({
            where: {
                projectId,
        ...
        // 🚀 清除剧集列表缓存
        try {
            await index_1.redis.del(`episodes:${projectId}`);
        }
        catch { }
...
const updateEpisode = async (req, res) => {
    try {
        ...
        const updatedEpisode = await index_1.prisma.episode.update({
            where: { id: episodeId },
            data: {
                name,
                description,
...
const deleteEpisode = async (req, res) => {
    try {
        ...
        await index_1.prisma.episode.delete({
            where: { id: episodeId },
        });
        // 🚀 清除剧集列表缓存
        try {
...
const getEpisodeCollaborators = async (req, res) => {
    try {
        const { projectId, episodeId } = req.params;
        const userId = req.user?.id;
        // 验证项目所有者权限
        ...
        // 获取项目的所有协作者
        const projectShares = await index_1.prisma.projectShare.findMany({
            where: { projectId },
            include: {
        ...
        // 获取该剧集的权限设置
        const episodePermissions = await index_1.prisma.episodePermission.findMany({
            where: { episodeId },
        });
...
exports.getEpisodeCollaborators = getEpisodeCollaborators;
// 更新剧集协作者权限
const updateEpisodePermission = async (req, res) => {
    try {
        const { projectId, episodeId } = req.params;
        ...
        if (permission === 'READ') {
            // READ是默认权限，删除记录即可
            await index_1.prisma.episodePermission.deleteMany({
                where: { episodeId, userId: targetUserId },
            });
        }
        else {
            // EDIT权限需要创建或更新记录
            await index_1.prisma.episodePermission.upsert({
...
exports.updateEpisodePermission = updateEpisodePermission;
