...
function sanitizeWorkflowNodes(nodes) {
     ...
     */
    const cleanBase64FromObject = (obj) => {
        ...
        for (const [key, value] of Object.entries(obj)) {
            if (typeof value === 'string' && value.startsWith('data:image')) {
                ...
                cleaned[key] = '';
                cleanedCount++;
                logger_1.logger.warn(`[Workflow] 清理了字段 "${key}" 中的 base64 图片数据`);
            }
    ...
    if (cleanedCount > 0) {
        logger_1.logger.warn(`[Workflow] 总共清理了 ${cleanedCount} 个 base64 图片字段，请使用 URL 而非 base64`);
    }
    return sanitized;
}
/**
