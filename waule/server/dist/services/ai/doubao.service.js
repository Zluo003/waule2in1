 ...
 */
async function processImageData(imageData, index) {
    ...
    if (imageData.url) {
        ...
        try {
            const ossUrl = await (0, oss_1.downloadAndUploadToOss)(imageData.url, 'doubao-image');
...
async function generateImage(options) {
    const { prompt, modelId, aspectRatio = '1:1', referenceImages = [], apiKey, apiUrl, maxImages = 1, // 默认单图
     } = options;
    // API配置
    ...
    try {
        ...
        const response = await axios_1.default.post(`${BASE_URL}/images/generations`, requestBody, {
            headers: {
 ...
 * 将视频比例转换为豆包API支持的格式
 */
function convertVideoAspectRatio(ratio) {
    const ratioMap = {
        '21:9': '21:9',
        '16:9': '16:9',
        '4:3': '4:3',
 ...
 * 将视频分辨率转换为豆包API支持的格式
 */
function convertVideoResolution(resolution) {
    const resolutionMap = {
        '720P': '720p',
        '1080P': '1080p',
        '2K': '2k',
...
async function generateVideo(options) {
    ...
    try {
        ...
        // 详细日志：检查图片URL格式
        requestBody.content.forEach((c, i) => {
            if (c.type === 'image_url') {
        ...
        console.log('📤 视频生成请求体:', JSON.stringify({
            ...requestBody,
            content: requestBody.content.map((c) => c.type === 'image_url' ? { ...c, image_url: { url: '[BASE64/URL]' } } : c)
        }, null, 2));
        const response = await axios_1.default.post(apiUrl_final, requestBody, {
            headers: {
                'Authorization': `Bearer ${API_KEY}`,
                'Content-Type': 'application/json',
            },
 ...
 */
async function pollContentGenerationTask(taskId, endpoint, apiKey, maxAttempts = 120) {
