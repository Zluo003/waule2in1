...
class VerificationCodeService {
    ...
    async initRedis() {
        ...
        try {
            ...
            logger_1.logger.info('验证码服务使用Redis存储 (集群模式安全)');
        }
        catch (error) {
            logger_1.logger.warn('Redis连接失败，使用内存存储验证码 (集群模式下可能导致验证失败)', error);
            this.useRedis = false;
            this.initialized = true;
            // 定期清理过期的内存验证码
     ...
     * 保存验证码（5分钟有效期）
     */
    async saveCode(phone, code, ttl = 300) {
        await this.ensureInitialized();
        const key = `sms:code:${phone}`;
        if (this.useRedis && this.redis) {
            await this.redis.setex(key, ttl, code);
            logger_1.logger.info(`验证码已保存到Redis: ${phone} (有效期${ttl}秒)`);
        }
        else {
            const expiresAt = Date.now() + ttl * 1000;
            this.memoryStore.set(key, { code, expiresAt });
            logger_1.logger.warn(`验证码已保存到内存: ${phone} (集群模式下可能失效)`);
        }
    }
    /**
     * 验证验证码（带尝试次数限制）
    ...
    async verifyCode(phone, code) {
        ...
        if (attempts >= this.MAX_VERIFY_ATTEMPTS) {
            logger_1.logger.warn(`验证码验证失败: ${phone} - 尝试次数过多 (${attempts})`);
            return false;
        }
        // 增加尝试次数
        ...
        if (this.useRedis && this.redis) {
            ...
            if (storedCode === code) {
                ...
                logger_1.logger.info(`验证码验证成功: ${phone}`);
                return true;
            }
        }
        else {
        ...
        logger_1.logger.warn(`验证码验证失败: ${phone} - 尝试次数 ${attempts + 1}/${this.MAX_VERIFY_ATTEMPTS}`);
        return false;
    }
    /**
     * 获取验证尝试次数
