...
class BillingService {
    ...
    async calculateCredits(params) {
        ...
        if (!rule) {
            logger_1.logger.debug('[BillingService] No billing rule found for params:', JSON.stringify(params));
            // 默认计费逻辑：如果没有配置规则，根据类型使用默认值
            // 视频生成：每秒 10 积分
            if (params.duration && params.duration > 0) {
                const defaultCredits = params.duration * 10;
                logger_1.logger.debug(`[BillingService] 使用默认视频计费: ${params.duration}秒 x 10积分 = ${defaultCredits}积分`);
                return defaultCredits;
            }
            // 图片生成：每张 20 积分
            if (params.quantity && params.quantity > 0) {
                const defaultCredits = params.quantity * 20;
                logger_1.logger.debug(`[BillingService] 使用默认图片计费: ${params.quantity}张 x 20积分 = ${defaultCredits}积分`);
                return defaultCredits;
            }
            logger_1.logger.debug('[BillingService] 无法计算积分：没有duration或quantity参数');
            return 0;
        }
        logger_1.logger.debug('[BillingService] 找到计费规则:', {
            id: rule.id,
            name: rule.name,
        ...
        if (!rule.isActive) {
            logger_1.logger.debug('[BillingService] Billing rule is inactive:', rule.id);
            return 0;
        }
        let credits = 0;
        switch (rule.billingType) {
            case client_1.BillingType.PER_REQUEST:
                credits = this.calculatePerRequest(rule, params);
                break;
            case client_1.BillingType.PER_IMAGE:
                credits = this.calculatePerImage(rule, params);
                break;
            case client_1.BillingType.PER_DURATION:
                credits = this.calculatePerDuration(rule, params);
                break;
            case client_1.BillingType.DURATION_RESOLUTION:
                credits = await this.calculateDurationResolution(rule, params);
                break;
            case client_1.BillingType.PER_CHARACTER:
                credits = await this.calculatePerCharacter(rule, params);
                break;
            case client_1.BillingType.DURATION_MODE:
                credits = await this.calculateDurationMode(rule, params);
                break;
            case client_1.BillingType.OPERATION_MODE:
                credits = await this.calculateOperationMode(rule, params);
                break;
            default:
                logger_1.logger.debug('[BillingService] Unsupported billing type:', rule.billingType);
                // 如果没有匹配的计费类型，使用 baseCredits
                credits = rule.baseCredits || 0;
        }
        logger_1.logger.debug('[BillingService] 计算结果:', { billingType: rule.billingType, credits });
    ...
    async chargeUser(params) {
        logger_1.logger.debug('[BillingService] chargeUser 收到参数:', JSON.stringify(params));
        // 🚀 优化：一次查询同时获取积分和规则
        const { credits, rule } = await this.calculateCreditsWithRule(params);
        logger_1.logger.debug('[BillingService] 计算积分结果:', credits);
        if (credits === 0) {
            logger_1.logger.debug('[BillingService] 积分为0，不扣费。参数:', JSON.stringify(params));
            return null;
        }
        // 检查用户积分
        ...
        try {
            ...
            // 🚀 清除用户资料缓存（积分已变化）
            try {
                await index_1.redis.del(`user:profile:${params.userId}`);
            }
            catch { }
            logger_1.logger.debug(`[BillingService] 已扣除 ${credits} 积分，用户: ${params.userId}`);
        }
        catch (error) {
        ...
        // 尝试创建使用记录（可选，表可能不存在）
        let usageRecord = { id: 'no-record', creditsCharged: credits };
        try {
            // 🚀 优化：复用上面已查询的 rule，不再重复查询
            usageRecord = await index_1.prisma.usageRecord.create({
                data: {
            ...
            logger_1.logger.debug('[BillingService] 创建使用记录失败（可能表不存在）:', e.message);
            // 继续，返回模拟记录
        }
        // 创建积分流水记录（消费）
        try {
            await index_1.prisma.creditTransaction.create({
                data: {
                    userId: params.userId,
                    type: 'CONSUME',
                    amount: -credits,
            ...
            logger_1.logger.debug('[BillingService] 创建积分流水失败:', e.message);
        }
        return usageRecord;
    }
    /**
     ...
     * 退还积分（任务失败时）
     */
    async refundCredits(usageRecordId, reason = '任务失败') {
        const usageRecord = await index_1.prisma.usageRecord.findUnique({
        ...
        if (creditsToRefund === 0) {
            logger_1.logger.debug('No credits to refund');
            return null;
        }
        // 使用事务退还积分
        ...
        // 🚀 清除用户资料缓存（积分已变化）
        try {
            await index_1.redis.del(`user:profile:${usageRecord.userId}`);
        }
        catch { }
        logger_1.logger.debug(`Refunded ${creditsToRefund} credits to user ${usageRecord.userId}, reason: ${reason}`);
        return result;
    }
    /**
     * 带权限检查的扣费（推荐使用）
     ...
     */
    async chargeUserWithPermission(params) {
        ...
        // 2. 检查并发限制
        const concurrencyResult = await user_level_service_1.userLevelService.checkConcurrencyLimit(userId);
        ...
        logger_1.logger.debug(`Charged ${isFreeUsage ? 0 : creditsToCharge} credits from user ${userId} (free: ${isFreeUsage})`);
        return {
            success: true,
            usageRecord: result,
            creditsCharged: isFreeUsage ? 0 : creditsToCharge,
    ...
    async checkPermissionOnly(params) {
        // 检查权限
        const permissionResult = await user_level_service_1.userLevelService.checkPermission(params);
        if (!permissionResult.allowed) {
            return {
                allowed: false,
    ...
    async getBillingRule(params) {
        ...
        const rule = await index_1.prisma.billingRule.findFirst({
            where,
            include: {
                prices: {
                    where: { isActive: true },
        ...
        if (rule) {
            try {
                await index_1.redis.set(cacheKey, JSON.stringify(rule), 'EX', 600);
            }
            catch {
                // Redis 写入失败，忽略
            }
     ...
     * 按次计费（文本模型）
     */
    calculatePerRequest(rule, params) {
        return rule.baseCredits;
    }
    /**
     * 按图片数量计费
     */
    calculatePerImage(rule, params) {
        const quantity = params.quantity || 1;
        // 1. 检查是否配置了分辨率价格
        if (params.resolution && rule.prices && rule.prices.length > 0) {
            const resPrices = rule.prices.filter((p) => p.dimension === 'resolution');
            if (resPrices.length > 0) {
                // A. 优先尝试精确字符串匹配 (忽略大小写)
                const exactMatch = resPrices.find((p) => p.value.toLowerCase() === params.resolution.toLowerCase());
                if (exactMatch) {
                    logger_1.logger.debug(`Image resolution exact match: ${exactMatch.value}, Unit Price: ${exactMatch.creditsPerUnit}`);
                ...
                if (targetPixels > 0) {
                    // 最近邻匹配 (按像素总数)
                    const sorted = resPrices.map((p) => ({
                        ...p,
                        pixels: getPixels(p.value),
                        diff: Math.abs(getPixels(p.value) - targetPixels)
                    })).sort((a, b) => a.diff - b.diff);
                    const bestMatch = sorted[0];
                    logger_1.logger.debug(`Image resolution ${params.resolution} (${targetPixels}px) matched to ${bestMatch.value} (${bestMatch.pixels}px), Unit Price: ${bestMatch.creditsPerUnit}`);
     ...
     * 按时长计费（广告成片）
     */
    calculatePerDuration(rule, params) {
        if (!params.duration)
            return 0;
     ...
     * 按时长+分辨率计费（视频生成、智能超清）
     */
    async calculateDurationResolution(rule, params) {
        if (!params.duration || !params.resolution)
            return 0;
        // 1. 提取分辨率数值
        const extractResolution = (res) => {
        ...
        const allPrices = rule.prices.filter((p) => p.dimension === 'resolution');
        if (allPrices.length === 0) {
            logger_1.logger.debug(`No resolution prices configured for this rule`);
            return 0;
        }
        const uniqueResolutions = Array.from(new Set(allPrices.map((p) => p.value.split('_')[0])));
        // 3. 智能匹配分辨率 (最近邻)
        ...
        const durationPrices = allPrices.filter((p) => p.value.startsWith(`${matchedRes}_`));
        if (durationPrices.length > 0) {
            // === 阶梯计费逻辑 ===
            // 提取时长并排序
            const durationOptions = durationPrices.map((p) => {
            ...
            let matchedOption = durationOptions.find((opt) => opt.duration >= params.duration);
            // 如果没有更大的，使用最大的 (兜底)
            if (!matchedOption) {
                matchedOption = durationOptions[durationOptions.length - 1];
            }
            logger_1.logger.debug(`Resolution ${params.resolution} matched to ${matchedRes}, Duration ${params.duration} matched to ${matchedOption.duration}s (Tiered), Cost: ${matchedOption.price.creditsPerUnit}`);
            return matchedOption.price.creditsPerUnit;
        }
        else {
            // === 线性计费逻辑 (回退到每秒单价) ===
            const priceConfig = allPrices.find((p) => p.value === matchedRes);
            if (!priceConfig) {
                logger_1.logger.debug(`Price config not found for resolution ${matchedRes}`);
                return 0;
            }
            const config = rule.config;
            const roundUp = config?.roundUp ?? true;
            const duration = roundUp ? Math.ceil(params.duration) : params.duration;
            logger_1.logger.debug(`Resolution ${params.resolution} matched to ${matchedRes} (Linear), Unit Price: ${priceConfig.creditsPerUnit}`);
     ...
     * 按字符数计费（音频合成）
     */
    async calculatePerCharacter(rule, params) {
     ...
     */
    async calculateDurationMode(rule, params) {
        if (!params.mode)
            return 0;
        const config = rule.config;
        const pricingUnit = config?.pricingUnit || 'per_second'; // 'per_second' | 'per_request'
        // 查找模式对应的价格
        let modePrice = rule.prices.find((p) => p.dimension === 'mode' && p.value === params.mode);
        // 如果没有精确匹配，使用默认模式或第一个模式
        if (!modePrice) {
            const modePrices = rule.prices.filter((p) => p.dimension === 'mode');
            if (modePrices.length === 0) {
                logger_1.logger.debug(`No mode prices configured for this rule`);
                return 0;
            }
            // 优先使用 'std' 或 'standard' 模式，否则使用第一个配置的模式
            modePrice = modePrices.find((p) => p.value === 'std' || p.value === 'standard') || modePrices[0];
            logger_1.logger.debug(`Mode ${params.mode} not found, using default: ${modePrice.value}`);
        }
     ...
     */
    async calculateOperationMode(rule, params) {
        // 使用传入的操作类型和模式，或使用默认值（Imagine + Relax）用于估算显示
        const operationType = params.operationType || 'Imagine';
        const mode = params.mode || 'Relax';
        logger_1.logger.debug('[calculateOperationMode] 输入参数:', { operationType, mode });
        logger_1.logger.debug('[calculateOperationMode] 规则价格:', JSON.stringify(rule.prices.map((p) => ({ dimension: p.dimension, value: p.value, creditsPerUnit: p.creditsPerUnit }))));
        // 查找操作类型的价格
        const operationPrice = rule.prices.find((p) => p.dimension === 'operationType' && p.value === operationType);
        if (!operationPrice) {
            // 如果找不到指定的操作类型，尝试获取任意一个操作类型价格
            const anyOperationPrice = rule.prices.find((p) => p.dimension === 'operationType');
            if (!anyOperationPrice) {
                logger_1.logger.debug(`No operationType price found for rule: ${rule.name}`);
                return rule.baseCredits || 0;
            }
            return anyOperationPrice.creditsPerUnit;
        }
        // 查找模式的倍率
        const modePrice = rule.prices.find((p) => p.dimension === 'mode' && p.value === mode);
        if (!modePrice) {
            // 如果找不到模式，只返回操作类型价格
     ...
     * 生成消费描述
     */
    generateConsumeDescription(params) {
        const parts = [];
        // 根据操作类型生成描述
        if (params.operation) {
            const operationMap = {
