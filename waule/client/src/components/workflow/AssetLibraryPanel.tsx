    ...
    try {
      ...
      if (isRoleLib) {
        ...
        for (const role of roles) {
          ...
          const ids = [m.images?.frontAssetId, m.images?.sideAssetId, m.images?.backAssetId, m.images?.faceAssetId].filter(Boolean) as string[];
          const imgs: { id: string; url: string; name: string }[] = [];
          for (const aid of ids) {
            try {
              const resp = await apiClient.assets.getById(aid);
              const asset = resp.data;
              imgs.push({ id: asset.id, url: resolveUrl(asset.url), name: asset.name });
            } catch {}
          }
            ...
            <>
              <div className="p-4 space-y-3">
                <div className="grid grid-cols-[96px,1fr] gap-2 items-center">
                  <label className="text-sm font-medium text-text-light-secondary dark:text-text-dark-secondary whitespace-nowrap">资产类型</label>
                ...
                <div className="grid grid-cols-[96px,1fr] gap-2 items-center">
                  <label className="text-sm font-medium text-text-light-secondary dark:text-text-dark-secondary whitespace-nowrap">资产库</label>
                  {(() => {
                      ...
                      <div className="grid grid-cols-3 gap-1.5">
                        {roleGroups.map((rg) => (
                          <div
                            ...
                            className="w-[150px] h-[150px] bg-card-light dark:bg-card-dark border border-border-light dark:border-border-dark rounded-lg overflow-hidden cursor-pointer hover:ring-2 hover:ring-tiffany-400 hover:scale-105 transition-all duration-200 group relative shadow-md"
                          >
                            {rg.coverUrl ? (
