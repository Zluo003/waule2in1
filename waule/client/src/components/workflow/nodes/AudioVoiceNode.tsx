...
const AudioVoiceNode = ({ data, id, selected }: NodeProps<AudioVoiceNodeData>) => {
  ...
  const loadSavedVoices = async () => {
    try {
      const resp = await apiClient.get('/ai/audio/voices');
  ...
  return (
    <div className={`relative bg-white dark:bg-[#18181b] backdrop-blur-xl border rounded-2xl shadow-xl transition-all ring-1 ${selected ? 'border-neutral-400 shadow-neutral-400/50' : 'border-white/60 dark:border-white/10 ring-white/5 dark:ring-white/5 ring-black/5'}`} style={{ width: 320 }}>
      ...
      <div className="p-4 space-y-4">
        ...
        <button onClick={handleSynthesize} disabled={isBusy || (data as any)._canEdit === false} className={`nodrag w-full mt-2 py-2 text-[10px] font-bold rounded-lg border transition-all active:scale-95 flex items-center justify-center gap-2 ${isBusy ? 'bg-neutral-300 dark:bg-neutral-700 text-neutral-500 dark:text-neutral-400' : 'bg-neutral-800 dark:bg-white text-white dark:text-black text-white shadow-md hover:shadow-lg border-transparent dark:border-white/10'}`}>
          {isBusy ? '合成中...' : '生成语音'}
        </button>
