...
const AudioPreviewNode = ({ data, id }: NodeProps<NodeData>) => {
  ...
  const computedSrc = useMemo(() => {
    ...
    return src;
  }, [data.audioUrl, data.sourceNodeId, apiBase]);
  useEffect(() => {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const downloadPath = `/assets/proxy-download-with-name?url=${encodeURIComponent(data.audioUrl)}&filename=${encodeURIComponent(fileName)}`;
  ...
  return (
    <div className="relative group" style={{ width: 360 }}>
      ...
      {/* 音频容器 */}
      <div className="relative bg-white dark:bg-[#18181b] backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-xl shadow-xl overflow-hidden py-4 px-3">
