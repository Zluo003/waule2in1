...
const VoiceCloneNode = ({ data, id, selected }: NodeProps<NodeData>) => {
    ...
    const handleCreate = async () => {
        ...
        try {
            ...
            if (sUrl) {
                updateNode({ sampleUrl: sUrl, status: 'OK', voiceName });
    ...
    return (
      ...
      <NodeCreatorBadge createdBy={(data as any).createdBy} isSharedWorkflow={(data as any)._isSharedWorkflow} />
            ...
            <div className="p-4 space-y-4">
                ...
                {/* Status Indicators */}
                <div className="space-y-2 bg-slate-100/50 dark:bg-white/5 p-3 rounded-lg border border-slate-200 dark:border-white/10">
                ...
                {/* Prompt Text */}
                <div className="space-y-1">
                    <label className="text-[10px] uppercase font-bold tracking-wider text-slate-400 dark:text-white/50">提示文本 (可选)</label>
                    <textarea
                        value={promptText}
