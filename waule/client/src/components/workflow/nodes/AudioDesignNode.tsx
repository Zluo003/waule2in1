      ...
      <div className="p-4 space-y-4">
        ...
        <div className="space-y-1">
          ...
          <input
            ...
            placeholder="定义音色名称"
            className="nodrag w-full p-2 text-xs rounded-md border outline-none transition-colors bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 focus:bg-white dark:focus:bg-white/10 border-slate-200 dark:border-white/10 focus:border-neutral-400 dark:focus:border-neutral-400/50 text-slate-800 dark:text-white placeholder-slate-400 dark:placeholder-white/30"
          />
        </div>
        <div className="space-y-1">
        ...
        <div className="flex items-center gap-2">
          <button onClick={handleDesign} disabled={isBusy || (data as any)._canEdit === false} className={`nodrag flex-1 py-2 text-[10px] font-bold rounded-lg border transition-all active:scale-95 flex items-center justify-center gap-2 ${isBusy || (data as any)._canEdit === false ? 'bg-neutral-300 dark:bg-neutral-700 text-neutral-500 dark:text-neutral-400' : 'bg-neutral-800 dark:bg-white text-white dark:text-black text-white shadow-md hover:shadow-lg border-transparent dark:border-white/10'}`}>
            {isBusy ? '设计中...' : '设计音色'}
          </button>
          <button onClick={handleSaveVoice} disabled={isBusy || !voiceId || (data as any)._canEdit === false} className={`nodrag px-3 py-2 text-[10px] font-bold rounded-lg border transition-all active:scale-95 ${isBusy || !voiceId || (data as any)._canEdit === false ? 'bg-neutral-300 dark:bg-neutral-700 text-neutral-500 dark:text-neutral-400' : 'bg-neutral-800 dark:bg-white text-white dark:text-black text-white shadow-md hover:shadow-lg border-transparent dark:border-white/10'}`}>
            保存
          </button>
        </div>
      </div>
