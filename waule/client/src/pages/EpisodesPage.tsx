...
const EpisodesPage = () => {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const episodeData = {
        ...formData,
        name: `第${formData.episodeNumber}集`,
  ...
  const openPermissionModal = async (episode: Episode) => {
    ...
    try {
      const response = await apiClient.episodes.getCollaborators(projectId!, episode.id);
  ...
  const handleUpdatePermission = async (userId: string, permission: 'READ' | 'EDIT') => {
    ...
    try {
      await apiClient.episodes.updatePermission(projectId!, selectedEpisode.id, userId, permission);
                  ...
                  {collaborators.map((collab) => (
                    ...
                    >
                      ...
                      <select
                        ...
                        onChange={(e) => handleUpdatePermission(collab.id, e.target.value as 'READ' | 'EDIT')}
