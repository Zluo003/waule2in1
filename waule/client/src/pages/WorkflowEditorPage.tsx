...
const WorkflowEditorInner = () => {
  ...
  const handleSocketNodeAdd = useCallback((node: Node, userId: string) => {
    ...
    setNodes((nds) => {
      ...
      // 添加模型数据
      return [...nds, { ...node, data: { ...node.data, models: aiModels } }];
    });
  }, [aiModels, setNodes]);
  ...
  const handleSocketNodeUpdate = useCallback((nodeId: string, changes: any, userId: string) => {
    ...
    // 短暂延时后移除标记
    setTimeout(() => remoteUpdatingNodesRef.current.delete(nodeId), 100);
  }, [setNodes]);
  ...
  const {
    ...
    workflowId: actualWorkflowIdRef.current,
    isSharedWorkflow,
    isReadOnly,
  ...
  // 规则：
  // 1. 工作流所有者可以编辑所有节点
  // 2. 分镜工作流：有编辑权限的协作者可以编辑所有节点
  // 3. 快速创作工作流：协作者只能编辑自己创建的节点
  // 4. 编组内的节点对协作者只读
  const canEditNode = useCallback((node: Node) => {
    ...
    // 分镜工作流：有编辑权限的协作者可以编辑所有节点
    if (isShotWorkflow) return true;
    
    // 快速创作工作流：协作者只能编辑自己创建的节点
    ...
    return creatorId === currentUserIdRef.current;
  }, [isWorkflowOwner, isShotWorkflow]);
  ...
  const nodesWithPermission = useMemo(() => {
    ...
    return nodes.map(node => {
      ...
      // 编组内节点：所有人都不能编辑（仅能下载/保存素材）
      const canEdit = isGrouped ? false : (isWorkflowOwner ? true : canEditNode(node));
      
      return {
        ...node,
        // 编组内节点：允许拖动（拖动时移动整个编组），但禁止连接
        draggable: node.draggable !== false,
        connectable: isGrouped ? false : (node.connectable !== false),
        data: {
          ...node.data,
          _canEdit: canEdit,
  ...
  const loadWorkflow = async () => {
    try {
      ...
      // 检查是否为协作者（只读模式）
      // 只读条件：
      // 1. canEdit 明确为 false（剧集协作者无编辑权限）
      // 2. canEdit 未定义 且 isOwner 为 false（快速创作共享工作流，始终只读）
      const shouldBeReadOnly = workflow.canEdit === false || 
        (workflow.canEdit === undefined && workflow.isOwner === false);
      ...
      // 分镜工作流：有编辑权限的协作者拥有和所有者相同的权限
      const hasOwnerPermission = workflow.isOwner !== false || (isShotWorkflow && workflow.canEdit === true);
      ...
      actualWorkflowIdRef.current = workflow.id;
      
      if (isShared) {
        console.log('[Workflow] 共享工作流，将启用自动刷新');
      }
  ...
  const saveWorkflow = async () => {
    ...
    try {
      ...
      if (isShotWorkflow) {
        await apiClient.workflows.saveShot(projectId!, episodeId!, sceneParam!, shotParam!, {
        ...
        try {
          await syncWorkflowToStoryboard(nodesRef.current, sceneParam!, shotParam!);
        } catch (error) {
          console.error('同步到分镜脚本失败:', error);
        }
      } else if (isEpisodeWorkflow) {
        await apiClient.workflows.saveEpisode(projectId!, episodeId!, {
          nodes: optimizedNodes,
          edges: optimizedEdges,
          nodeGroups: nodeGroupsRef.current,
          viewport: { x: 0, y: 0, zoom: 1 },
        });
      } else if (isDirectWorkflowAccess && directWorkflowId) {
        // 直接访问模式（所有者）保存
        await apiClient.workflows.update(directWorkflowId, {
          data: {
            nodes: optimizedNodes,
            edges: optimizedEdges,
            nodeGroups: nodeGroupsRef.current,
        ...
        // 保存项目工作流
        if (id) {
          await apiClient.workflows.save(id, {
            nodes: optimizedNodes,
            edges: optimizedEdges,
            nodeGroups: nodeGroupsRef.current,
            viewport: { x: 0, y: 0, zoom: 1 },
  ...
  const loadAIModels = async () => {
    try {
      const response = await apiClient.get('/ai/models', { params: { isActive: 'true' } });
  ...
  useEffect(() => {
    setNodes((nds) => nds.map((node) => {
      ...
      if (!t.startsWith('aiVideo') && t !== 'audioVoice' && t !== 'voiceClone' && t !== 'audioSynthesize' && t !== 'audioDesign') return node;
      return {
        ...node,
        data: {
          ...node.data,
    ...
    try {
      ...
      for (const n of Array.isArray(_deleted) ? _deleted : []) {
        ...
        if (n?.type === 'imagePreview' && n?.data?.imageUrl) {
          tasks.push(apiClient.post('/assets/recycle/record', { url: n.data.imageUrl, type: 'IMAGE', projectName }));
        } else if (n?.type === 'videoPreview' && n?.data?.videoUrl) {
          tasks.push(apiClient.post('/assets/recycle/record', { url: n.data.videoUrl, type: 'VIDEO', projectName }));
        }
  ...
  type ConnectionMenuFilters = { showAgent: boolean; showAiImage: boolean; showAudio: boolean; showVideo: boolean; showEdit?: boolean; showImageEditing?: boolean; showMidjourney: boolean; showUpload: boolean; showAssetSelector: boolean; showSuperCanvas?: boolean };
  const connectionMenuFilters: ConnectionMenuFilters = useMemo(() => {
    const defaults: ConnectionMenuFilters = { showAgent: true, showAiImage: true, showAudio: true, showVideo: true, showEdit: true, showImageEditing: true, showMidjourney: true, showUpload: true, showAssetSelector: true, showSuperCanvas: true };
    ...
    return {
      ...
      showImageEditing: outputType === 'IMAGE',
