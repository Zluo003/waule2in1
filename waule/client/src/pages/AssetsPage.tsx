...
const LibraryModal: React.FC<LibraryModalProps> = ({
  ...
  const handleAddCollaborator = async (user: any) => {
    ...
    try {
      ...
      setCollaborators(prev => [...prev, { id: user.id, nickname: user.nickname, avatar: user.avatar, canDownload: true }]);
      setSearchQuery('');
      setSearchResults([]);
      setShowSearchDropdown(false);
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className={`bg-white/90 dark:bg-black/70 backdrop-blur-xl border-2 border-slate-200 dark:border-white/10 rounded-2xl p-6 ${isEdit ? 'w-[800px]' : 'w-[640px]'} max-w-[95vw] shadow-lg max-h-[90vh] overflow-y-auto`}>
          ...
          <div className={isEdit ? 'grid grid-cols-2 gap-4' : ''}>
            ...
            {isEdit && (
              <div>
                ...
                <div className="aspect-[16/9] border-2 border-slate-200 dark:border-white/10 rounded-md bg-slate-100 dark:bg-white/5 p-2 flex flex-col">
                  {/* 搜索框 */}
...
const AssetsPage = () => {
  ...
  const loadSoraCharacterCount = async () => {
    try {
      ...
      setSoraCharacterCount((response.characters || []).length);
      
      // 同时加载共享信息
      const shareInfo = await apiClient.soraCharacters.getShareInfo();
      setSoraHasCollaborators(shareInfo.data?.hasCollaborators || false);
    } catch (error) {
      console.error('加载Sora角色数量失败:', error);
    }
  };
  ...
  const handleAddSoraCollaborator = async (user: any) => {
    try {
      ...
      setSoraCollaborators(prev => [...prev, { id: user.id, nickname: user.nickname, avatar: user.avatar }]);
      setSoraSearchQuery('');
      setSoraSearchResults([]);
      setShowSoraSearchDropdown(false);
