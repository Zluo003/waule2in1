  ...
  const handleSearchUsers = async (query: string) => {
    ...
    searchTimeoutRef.current = setTimeout(async () => {
      ...
      try {
        ...
        // 管理员可以看到"所有人"选项（仅 QUICK 项目，且未公开）
...
const ProjectsPage = () => {
  ...
  const activeTab: 'ALL' | 'QUICK' | 'DRAMA' =
    window.location.pathname.startsWith('/quick')
      ? 'QUICK'
      : window.location.pathname.startsWith('/drama')
        ? 'DRAMA'
        : 'ALL';
  ...
  const loadProjects = async () => {
    try {
      ...
      if (activeTab === 'DRAMA' || activeTab === 'ALL') {
        try {
          const sharedRes = await apiClient.projects.getShared(activeTab === 'DRAMA' ? { type: 'DRAMA' } : {});
          const sharedDramaProjects: Project[] = (sharedRes.data || [])
            .filter((p: any) => p.type === 'DRAMA')
            .map((p: any) => ({
              ...p,
              isOwner: false,
              isShared: true,
