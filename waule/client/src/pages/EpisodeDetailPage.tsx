...
export default function EpisodeDetailPageNew() {
    ...
    const handleBeforeUnload = () => {
      ...
      if (currentData !== lastSavedRef.current && shots.length > 0) {
        ...
        navigator.sendBeacon?.(
          `${import.meta.env.VITE_API_URL || ''}/api/projects/${projectId}/episodes/${episodeId}`,
  ...
  const enterWorkflow = async () => {
    ...
    if (currentShot?.selectedAudios) {
      currentShot.selectedAudios.forEach((a: any) => assets.push({ type: 'audio', ...a }))
    }
